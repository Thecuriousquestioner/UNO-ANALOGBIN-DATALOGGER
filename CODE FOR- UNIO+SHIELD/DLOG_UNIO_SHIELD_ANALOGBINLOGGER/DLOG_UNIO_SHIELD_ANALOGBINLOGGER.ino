...
const float SAMPLE_RATE = 6000;  // Must be 0.25 or greater.
...
digitalWrite(greenLED,LOW);
  ...
  meta->timestamp = rtc.now().unixtime();
  // Write metadata.
  ...
  delay(1000);
  Serial.println(F("Logging - Press the RED Button to stop"));
...
//------------------------------------------------------------------------------
void loop(void) {
//  Serial.println(F("Standing by to Record data"));
//  delay(2000);
digitalWrite(ERROR_LED_PIN, LOW);
...
//  } while (Serial.available() && Serial.read() >= 0);
 ...
 if (digitalRead(startlog)==HIGH) {
    digitalWrite(greenLED,HIGH);
    logData();
   // binaryToCsv();
  } else {
//    Serial.println(F("Press the button when you are ready!!"));
//    delay(2000);
digitalWrite(greenLED, HIGH);
delay(500);
digitalWrite(greenLED,LOW);
